...
monitor_status(){
 while true; do
    ...
    #Global sleep after checks/restarts. Set for 5min in Prod
    sleep 2
    ...
    if [[ "$jobStatus" == "RUNNING" || \
     ...
     max_elapsed_time=1800  # 1800, 30 minutes in seconds
        ...
        while [[ "$jobStatus" == "STARTING" || \
         ...
         sleep 1
         #Adding 3 seconds into elapsed time to account for sleep + time to query below (change to 117 in prod)
         ((elapsed_time += 3))
     ...
     #Remove the below for prod and use the above
    elif [[ "$jobStatus" == "CANCELED" || \
